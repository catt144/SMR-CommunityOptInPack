...
| `03_Drones/` | Build 4: the hub's drones, **as a mini chain** (owner, 2026-09-22). Vanilla Wasps launched from the pit, repairing broken track out on the network from the hub's stock. Its `DESIGN.md` holds the settled design; numbered links pass evidence downstream. Its own `README.md` is the manifest. | **Link 5 needs the owner sitting.** First confirm template loading after the dwell rollback; `3a0faff` is withdrawn after owner load crashes. Audit report §8 holds evidence and the restart control. Save compatibility, the stall and full review remain open. Links 1-4 are consumed |
