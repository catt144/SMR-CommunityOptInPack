...
| `01_TRAIN_HUB_STRUCTURE_high.md` | **The hub's look, now.** Textures on everything except the finished road: ring, portals, ribs, supports, rails and trim. It fixes the blurry, jagged seams and rib bands, then adds the structure's own lights, then takes one GPU reading of the whole hub, on against off. | **Live, C run: fire in a fresh session** — start at its §"Where this stands — the C run" (2026-09-22 evening): the look is kept through the bays; the crown light, the yellow frames and the whole-hub cost reading are what remain |
...
| `03_Drones/` | Build 4: the hub's drones, **as a mini chain** (owner, 2026-09-22). Vanilla Wasps launched from the pit, repairing broken track out on the network from the hub's stock. Its `DESIGN.md` holds the settled design; its numbered links each outbox what the next needs, so the link that waits for the structure pass starts fully briefed. Its own `README.md` is the manifest. | **Link 3 is next** (attended: the owner flies the L2M2 motion build, `b84f106` or later, beside an ordinary Wasp; its step zero is the EF-116 control line); link 4 waits for `20_TrainHub.lua` |
