...
| `03_Drones/` | Build 4: the hub's drones, **as a mini chain** (owner, 2026-09-22). Vanilla Wasps launched from the pit, repairing broken track out on the network from the hub's stock. Its `DESIGN.md` holds the settled design; numbered links pass evidence downstream. Its own `README.md` is the manifest. | **Link 5 needs the owner sitting.** Start with its audit handoff: stalled-save diagnosis and native save check (`reports/TRAIN_HUB_AUDIT_111_20260923.md`). Restart the process for the dwell fix `3a0faff`. The stall and full compatibility review remain open. Links 1-4 are consumed; `20_TrainHub.lua` is free |
