...
| `03_Drones/` | Build 4: the hub's drones, **as a mini chain** (owner, 2026-09-22). Vanilla Wasps launched from the pit, repairing broken track out on the network from the hub's stock. Its `DESIGN.md` holds the settled design; numbered links pass evidence downstream. Its own `README.md` is the manifest. | **Link 5 in progress.** Rollback loads pass and the owner confirms trains unstuck (audit §§8–9). The save-guard candidate passes local tests; native old→new→save→reload is pending (§10). Remaining smoke and full review stay open. Links 1-4 are consumed |
