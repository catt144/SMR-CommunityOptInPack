...
`00`, `02` to `05` and `07` are live (`01` is parked, done; the earlier `05`, the portal doors, was fired, built and then cut by the owner on
...
at a time: `02`, `03_Drones/`'s link 4 and `04`; `05` touches no Lua but its one palette default. The drones chain's
