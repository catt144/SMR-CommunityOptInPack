...
| `03_Drones/` | Build 4: the hub's drones, **as a mini chain** (owner, 2026-09-22). Vanilla Wasps launched from the pit, repairing broken track out on the network from the hub's stock. Its `DESIGN.md` holds the settled design; its numbered links each outbox what the next needs, so the link that waits for the structure pass starts fully briefed. Its own `README.md` is the manifest. | Route settled (L2R); **link 3 BLOCKED again, 2026-09-22** on a rebuilt link 2 for flight motion/banking (`03_Drones/README.md`); link 4 waits for `20_TrainHub.lua` |
