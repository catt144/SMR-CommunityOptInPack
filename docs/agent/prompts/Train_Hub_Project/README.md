...
| `03_Drones/` | Build 4: the hub's drones, **as a mini chain** (owner, 2026-09-22). Vanilla Wasps launched from the pit, repairing broken track out on the network from the hub's stock. Its `DESIGN.md` holds the settled design; numbered links pass evidence downstream. Its own `README.md` is the manifest. | **Link 5 in progress.** Siding departures and the save guard pass their owner controls, including autosave/reload and a new manual save loaded after restart (audit §§8–12). Resume the remaining drone smoke; broader review stays open. Links 1-4 are consumed |
