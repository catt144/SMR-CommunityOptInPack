...
| `TRAIN_HUB_BUILD_high.md` | `prompt` | **LIVE, fire when ready.** The hub prototype was a qualified GO (owner, 2026-09-18). Build the real six-connector Module B hub on a stand-in body until the owner's asset lands: a shared base per size with the four's name reserved, a built-in small-radius drone controller, a Metals maintenance reserve that trains and drones leave alone, the prototype's five carried checks plus Waste Rock, and a sitting with the owner |
