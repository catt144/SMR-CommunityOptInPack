...
| `GEOMETRY_ORACLE_PATHS_high.md` | `prompt` | **LIVE, fire when ready.** Owner, 2026-09-20: the oracle is extended before build 3b is re-scoped. Its path model routes every train through `Enter1` in the connector's outward frame, which is neither the game nor the corrected table, so CHOREOGRAPHY, CLEARANCE and TWO-TRAIN print numbers nobody may use. Fold in the measured element-angle rule and the arrival/departure spot choice (`Train.lua:665`), re-run with `hub_connector_directions = {4,1,3,0,2,5}`, and report what 3b still has to solve. Desk work; it does not touch `20_TrainHub.lua`. Deleted when its deliverable lands |
| `TRAIN_HUB_TRAINS_high.md` | `prompt` | ⛔ **HOLD, re-scope before firing; build 4 waits behind it.** Its premise is contradicted by the geometry oracle's measured finding (`reports/GEOMETRY_ORACLE_20260919.md`: `hub_connector_directions` wrong for indices 1-4, lane offset 289 units); the brief's own hold block says what changes. Build 3b: trains at the hub (owner, 2026-09-19, from build 3's smoke): the stubs are the platforms, reverse in place, lanes measured, route through the centre with a turn, occupancy per stub plus a crossing lock, so trains never leave the deck or slide over open floor. Smoke only. Deleted when its smoke is recorded. |
| `TRAIN_HUB_REPAIR_high.md` | `prompt` | ⛔ **HELD until build 3b's smoke is recorded (`TRAIN_HUB_TRAINS_high.md`).** Build 4: the hub's on-demand repair drones (owner, 2026-09-19): vanilla Wasps under the hub, a constant 30, never charging; anything a drone does within 15 hexes, track repair along the connected network beyond it, paid from the hub's stock on a persisted deadline, with a save guard for track mode. Adds one persisted name |
