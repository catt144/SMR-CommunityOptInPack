...
| `TRAIN_HUB_LOOK_high.md` | `prompt` | **LIVE, fire when ready — the texture gate is LIFTED (owner, 2026-09-21).** Bring the owner's concept art to life on the final model: the body's four maps including the self-illum glow mask, glass on the six loading platforms as its own attached entity (the vanilla dome pattern; one mesh node takes one material), and a themed generator entity to replace the vanilla reactor visual. No dome glass, no body geometry or UV change, never a restyle of vanilla's own material. A first cut to look at in day and night, then iterate with the owner on the same brief |
