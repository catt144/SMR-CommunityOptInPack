...
| `TRAIN_HUB_PROTOTYPE_high.md` | `prompt` | **LIVE, fire when ready.** OI-10 ruled 2026-09-18 ("prototype B via 3a"): spike, then build an ugly hub `Station` with at least six connectors (three lines), test three-route interchange with the owner through SMRTK, and report a go/no-go for the owner's asset investment. Disposable saves only; no routing, no Module A |
