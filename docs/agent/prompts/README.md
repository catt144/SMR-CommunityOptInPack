...
| `TRAIN_HUB_MOVE_high.md` | `prompt` | **LIVE, fire when ready.** The transition and trains on the centre of our track (owner, 2026-09-20): a train stops as its tail clears the vanilla rail, slides onto our centre, runs in, loads, and mirrors it on the way out. Owns `20_TrainHub.lua`, carries the cold-start power fix and halves the loading dwell to 6 s for hub trains only (owner, 2026-09-20); retires build 3b. Function only — **no texture pass until the owner greens the whole cycle**. Smoke only; build 4 stays held |
