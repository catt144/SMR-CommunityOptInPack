...
| `GEOMETRY_ORACLE_high.md` | `prompt` | **LIVE, fire when ready — before build 3b.** Every train-hub failure so far has been a coordinate or transform mismatch, caught only by the owner's eyes after a manual re-import (owner, 2026-09-19). Build the harness that predicts, from files alone, what the game will do with the geometry: footprint, spots, cargo, train choreography, clearance, two-train overlap, lane alignment. It must retrodict the five failures already paid for, and its rules must be triangulated, not derived once. Predicts only; changes no geometry. Deleted when the harness and its report land. |
