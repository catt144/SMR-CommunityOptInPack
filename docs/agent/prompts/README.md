...
| `TRAIN_HUB_LOOK_high.md` | `prompt` | **LIVE, first concept cut prepared; owner import and day/night look next (2026-09-21).** Bring the owner's concept art to life on the final model: the body's four maps including the self-illum glow mask, glass on the six loading platforms as its own attached entity (the vanilla dome pattern; one mesh node takes one material), and a themed generator entity to replace the vanilla reactor visual. OI-23 first unwrap and glass split are frozen at the paired prepaint tags; no further body geometry or UV change, no dome glass, never a restyle of vanilla's own material. First cut imported 2026-09-21; iteration 1 is the centre z-fight (a geometry defect the paint exposed, owner-approved exception) and the relaid dash lights. Glass and reactor DEFERRED by the owner to a later session |
...
| `TRAIN_HUB_MODEL_high.md` | `prompt` | **LIVE, Pass 2 ready to fire.** Pass 1 (stubs one hex longer, twelve transition arms, footprint) is imported untextured and accepted in game, but uncommitted. Pass 2 (sidings) is imported and seen. **Pass 3, ready to fire:** the owner found a parked car overhangs the siding and the deck ends short, so the deck grows OUTWARD only (about 25.5 m long, 5.5 m wide) — the inner end and the hub's cargo beds do not move (owner, 2026-09-20). Rough is right: the owner eyes the gaps |
