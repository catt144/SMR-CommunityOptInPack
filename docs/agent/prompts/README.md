...
|---|---|---|
| `GEOMETRY_ORACLE_B_high.md` | `prompt` | **LIVE, fire when ready.** Run B of the geometry oracle (run A built and committed the instrument and its report, 2026-09-19): judge the vanilla `entities.dat` decoder's return against its anchors, run the oracle's LANE verdict with the measured 289-unit offset, take the train's bounding box, then close. Deleted when those land. |
