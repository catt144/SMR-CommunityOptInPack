...
| `RAIL_SHAFT_PROTOTYPE_high.md` | `prompt` | **LIVE, PARTLY CONSUMED — only the sitting is left** (owner, 2026-09-21; state 2026-09-23). The elevator bypass from `reports/ELEVATOR_LOGISTICS_OPTIONS.md` (option F): a train tunnel whose two ends sit on different maps. Research and prototype are DONE — `reports/RAIL_SHAFT_PROTOTYPE.md` and the dev mod `tools/devmods/rail_shaft/`. Stop (3) fired (the game patched to build 25390750), so the brief's own citations are superseded by the report's; do not redo the research. Fire it only to hold the sitting in report §6 and record the result in §5 |
