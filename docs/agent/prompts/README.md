...
| `TRAIN_HUB_TRAINS_high.md` | `prompt` | **LIVE, fire when ready** (re-scoped 2026-09-20: the owner set the step-0 gate aside and withdrew OI-22, `GEOMETRY_ORACLE_20260919.md` §13). Build 3b: the park distance becomes a tunable tuned by eye, plus the connector-table fix, spots on the lanes right of travel, reverse in place facing out, a centre turn that sets its own yaw, and the crossing lock. ⛔ The 41.5 m train length is DISPUTED — the owner's hex-grid measurement (~20 m) governs, and 3b owes the clean read. Owns `20_TrainHub.lua`; build 4 waits behind its smoke. Smoke only. Deleted when its smoke is recorded |
