...
|---|---|---|
| `RULES_CENSUS_high.md` | `prompt` | ⛔ **LIVE, NOT FIRED — RUN THIS FIRST.** Put every rule-shaped sentence through the donor's rule-placement test (*"what actually stops this, if not the reader's memory?"*) and place it by tier, or purge it (owner, 2026-09-17). ⛔ Sequenced **before** `PROVENANCE_DISSOLVE_high.md`, which lands into the surfaces this rewrites |
| `PROVENANCE_DISSOLVE_high.md` | `prompt` | ⛔ **LIVE, NOT FIRED — RUN SECOND**, after `RULES_CENSUS_high.md`. Dissolve `agent/PROVENANCE.md` (owner, 2026-09-17: *"I would honestly like it to just go away"*). Rehome §2 save contract to `support/`, §6 to `tools/README.md`, §4 to the suite docs; replace the 15,599 B port ledger with `tools/sync_facts.py` + a doccheck gate, because the fact mirror differs from the donor's in exactly **3 declared files**. ⚠️ Carries a stop: `items.lua` and `metadata.lua` cite §2 as save-contract authority and they ship |
