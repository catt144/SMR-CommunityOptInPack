...
| `TRAIN_HUB_LOOK_high.md` | `prompt` | **LIVE, PAUSED by the owner after iteration 1's import (2026-09-21).** Body and concept maps imported; glow works but the hub reads as flat "blue paint" — measured: normal 97.9% flat, base colour 84.2% one value, RM without metalness, at vanilla's own format and size. Next iteration HELD; the owner is trialling an external AI texturer on the bare exported model (spec §9). When it resumes: normal bake first. Glass and themed reactor stay deferred. Survives until owner accepts the look |
