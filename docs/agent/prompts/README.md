...
| `TRAIN_HUB_LIGHTS_medium.md` | `prompt` | **LIVE, fire when ready.** Owner, 2026-09-21: real lights instead of painted glow on the tracks and borders. Remove the road and platform strips (road finish B held), then our own Lua lights on the six arms, three reds and three blues, one variant per arm for the owner to choose in game, day and night, working-gated. Design smoke only. Deleted when fired |
