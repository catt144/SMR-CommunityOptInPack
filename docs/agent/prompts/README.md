...
| `TRAIN_HUB_TRAINS_high.md` | `prompt` | **LIVE, fire when ready** (re-scoped 2026-09-20 on the oracle's paths run, report §11; the hold is lifted). Build 3b: trains at the hub (owner, 2026-09-19, from build 3's smoke): the line is the platform, reverse in place facing out, spots on the lanes right of travel, route through the centre with a turn that sets the yaw itself, occupancy plus a crossing lock, so trains never leave the deck, crab sideways or slide over open floor. Carries the owner's 2026-09-20 ruling to move Stop outward and verify it with the oracle. Owns `20_TrainHub.lua`; build 4 waits behind it. Smoke only. Deleted when its smoke is recorded |
