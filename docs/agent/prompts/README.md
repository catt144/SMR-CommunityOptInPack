...
| `GEOMETRY_ORACLE_high.md` | `prompt` | **LIVE, fire when ready — before build 3b.** Every train-hub failure so far has been a coordinate or transform mismatch, caught only by the owner's eyes after a manual re-import (owner, 2026-09-19). Buy that loop out: find a geometry mistake from files on disk, before a re-import and before a sitting. The instrument, the framing and the scope are the worker's to choose or overturn; two standards are not — it must retrodict the five failures already paid for without tuning to fit, and a rule it trusts needs two independent derivations that agree. Measures only, changes no geometry. Deleted when the work and its report land. |
| `TRAIN_HUB_TRAINS_high.md` | `prompt` | **LIVE, fire when ready; build 4 waits behind it.** Build 3b: trains at the hub (owner, 2026-09-19, from build 3's smoke): the stubs are the platforms, reverse in place, lanes measured, route through the centre with a turn, occupancy per stub plus a crossing lock, so trains never leave the deck or slide over open floor. Smoke only. Deleted when its smoke is recorded. |
