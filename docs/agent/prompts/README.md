...
| `TRAIN_HUB_REPAIR_high.md` | `prompt` | ⛔ **HELD until build 3's smoke is recorded.** Build 4: the hub auto-dispatches a repair train to breaks on its network (owner, 2026-09-19): the hub pays the repair from stock and completes the site after a persisted, distance-based travel time; the train is a cosmetic vehicle with a repair livery, never a unit or a drone. Adds one persisted name |
