...
| `TRAIN_HUB_STRUCTURE_high.md` | `prompt` | **LIVE, fire when ready** (owner, 2026-09-21; the lights brief is fired and closed at restore point `hub-lines-b2-lights-20260921`, where this starts). Look step 2: the hub structure (ring, portals, ribs, supports, rails, trim) built to a premium, wonder-scale bar: BaseColor 4096 supersampled, other maps 2048, crisp panel seams, road and the held deep blue lines untouched by value; one importer run; restore point on the owner's keep. Then the structure's own Lua lights, and only then the whole hub's off/on GPU cost (owner: measuring now "would just be testing the tracks"). Deleted when fired |
| `TRAIN_HUB_MOVE_high.md` | `prompt` | **PARKED, do not fire** (owner, 2026-09-21: the movement work is finished; it may reopen at the final pre-launch test if the owner wants some moves slightly tweaked). Gates nothing. Kept for that test: the slide onto the loading siding, two jobs: delete the run-length speed scaling in `HubMoveOntoSiding` so the lateral rate matches the accepted outer slide, then move the onset later in the travel as a live tunable |
