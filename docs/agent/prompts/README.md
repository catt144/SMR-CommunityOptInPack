...
| `TRAIN_HUB_TRAINS_high.md` | `prompt` | ⛔ **HELD (owner sitting 2026-09-20, steps 1-3 only): the owner redirected trains onto the centre of our track, so the lane build's remaining smoke is not owed as written; the movement prototype that follows `TRAIN_HUB_MODEL_high.md` replaces it on the owner's word.** Was: owner visual sitting next. Build 3b implemented at a provisional 20 m park distance; unattended straight/60°/120°/reverse and parked-plus-crossing reload passed. Slots and next batch: `TRAIN_HUB_BUILD_20260918.md` §Build 3b. Owner still judges turning, portal joins and queue clearance. The clean parts/grid read leaves R-TRAIN DISPUTED (`GEOMETRY_ORACLE_20260919.md` §13); the owner's hex measurement governs. Owns `20_TrainHub.lua`; build 4 stays held. Smoke only. Deleted when the owner's smoke is recorded |
| `TRAIN_HUB_MODEL_high.md` | `prompt` | **LIVE, fire when ready.** The hub model, quick and inspectable (owner, 2026-09-20): each stub one hex longer, transition platform arms either side of a one-hex track path, footprint to match. Renders to the owner first, then the junction fix, a re-import and one in-game look. Movement is a later brief |
