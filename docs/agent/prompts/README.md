...
| `TRAIN_HUB_MODEL_high.md` | `prompt` | **LIVE, Pass 2 ready to fire.** Pass 1 (stubs one hex longer, twelve transition arms, footprint) is imported untextured and accepted in game, but uncommitted. Pass 2: bake the owner's 0.573 m platform offset into the generator FIRST, then six loading sidings (cantilevered, metal border with an opaque panel in the body mesh; the importer keeps one mesh node, measured), export and the owner's import. Rough is right: the owner eyes the gaps |
