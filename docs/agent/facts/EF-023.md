...
- **THE REAL RULE (measured 2026-07-31, PT-20 — supersedes the framing above):
  ...
  * ✅ RESOLVED 2026-08-01 by the Tier-1 rewrite: ~~the two shipped file headers~~
    ...
    unrelated (and correct) statement about `OnMsg` registrations. ⭐ In THIS repo the
    claim has always been true and was re-swept at the same date: this mod owns
    no game-time thread and no `Code/` file has ever stated the belief. The
    corrected sites are all in the fix pack's tree.
