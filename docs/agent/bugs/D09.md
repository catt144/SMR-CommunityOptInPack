...
## ✅ 2026-09-17 — 1.1.0: EVERY CITED MECHANISM SURVIVED UNCHANGED
...
revalidation pass. No code changed. Report:
`docs/agent/reports/MODULE_REVALIDATION_1_1_0.md` §7. Owner ask: none — nothing moved.

Of the seven modules re-read in that pass, this is the only one whose every load-bearing citation
still resolves to the same behaviour on 1.1.0:

* `const.DroneResourceCarryAmount` still defined with `value = 1` in the `Drone` group
  (`1.1.0.403908/Src/Lua/__const.lua:683-687`, identical to `1.0.7.396349/Src/Lua/__const.lua:637-641`),
  still consumed at `1.1.0.403908/Src/Lua/Units/Drone.lua:790`, still rebuilt on
  `OnMsg.ConstValueChanged` (`:793`).
* `Drone.move_speed` still a plain member (`1.1.0.403908/Src/Lua/Units/Drone.lua:28`, applied `:90`).
* `LabelContainer:SetLabelModifier` unchanged — the only diff in the whole file is a varargs
  pass-through on the unrelated `UpdateFilteredLabel` (`1.1.0.403908/Src/Lua/LabelContainer.lua:80-97`).
* `Modifier`, `HasModifiablePropScale`, `GetModifiablePropScale` all still in
  `1.1.0.403908/Src/CommonLua/Classes/Modifiers.lua`.
* The comparable techs still use the same props — `DroneResourceCarryAmount` at
  `1.1.0.403908/Src/Data/Tech.lua:478` (⚠️ CITATION MOVE ONLY: the preset file was renamed
  `Data/TechPreset.lua` → `Data/Tech.lua`; the module header's `Data/TechPreset.lua:626-630/:702-706`
  citations are 1.0.7 paths), and the Ancient Artifact upgrade route at
  `1.1.0.403908/Src/Lua/BuildingTemplate/AncientArtifactInterface.generated.lua:24`.

**⭐ THE MOD OPTIONS CONTRACT SURVIVED, AND IT UNDERWRITES ALL EIGHT MODULES, NOT JUST THIS ONE.**
The API moved file — `CommonLua/Classes/Mod.lua` → `1.1.0.403908/Src/CommonLua/Modding/Mod.lua` —
but is otherwise intact: `CurrentModOptions` is still injected into the mod env (`:1634`),
`Msg("ApplyModOptions", mod.id)` still fires on apply and on load (`:697, :753-759, :2199`), and
`ModItemOptionChoice:GetOptionMeta` still builds `{ text = T(item), value = item }` — **the choice
STRING is still the value** (`:2781-2789`, byte-identical to
`1.0.7.396349/Src/CommonLua/Classes/Mod.lua:2764-2771`), which is what this module's
`SPEED_PERCENT` / `CARRY_ADD` string keys depend on.

Vanilla 1.1.0 ships no drone speed or carry dial; the only levers are still the techs, which is
this module's premise.

**Control — what would falsify this.** On 1.1.0 set "Drone speed" to 5x, hit Apply, and read a
drone's `move_speed`. If it does not rise, either the modifier is not landing or the prop lost its
modifiable scale — and `ReapplyDials`' `HasModifiablePropScale` guard would have returned silently
rather than erroring, which reads as "the dial does nothing" rather than as a crash. Not run.

**What this does NOT claim.** Nothing about the no-clamp probe (2026-07-29, 1.0.7) — the C-side
clamp question was settled on a build that is no longer installed and the stacking headroom
argument rests on it. Nothing about the save-footprint behaviour of the two persisted `Modifier`
ids, which is unchanged in source but unverified on 1.1.0.
