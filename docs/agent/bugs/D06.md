...
(DECISIONS_OWED 92, E-4/E-8) were specified against the machinery above. Re-read both against the
1.1.0 tree before ratifying item 94.
...
**The ruling.** The owner ruled the `OI-05` ask (`docs/DECISIONS_OWED.md`, closed by that ruling):
...
(`../reports/DRONE_REBUILD_DESIGN_20260901.md`) and its build brief
(`../prompts/DRONE_REBUILD_BUILD.md`) were written against 1.0.7 machinery and have to be re-based
on 1.1.0 first, and a replacement for `CalcLapTime` has to exist before any claim about the module's
value can be measured at all. ⚠️ `DECISIONS_OWED.md` items **94** (ratify the rebuild spec) and
**92** (the E-4/E-8 matcher experiments) remain OPEN and are still specified against 1.0.7 — that
caveat was raised with OI-05 and survives its closure.
