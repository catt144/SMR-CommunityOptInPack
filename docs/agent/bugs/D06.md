...
seq: 6
row: 6
title: "Drone assignment has no cross-hub locality (far fleets claim near work) — ⚖️ RETIRED 2026-09-17 (PARKED), module no longer ships"
status: "built"
status_source: "tag"
priority: "dsgn"
evidence: "high"
row_status: "built (v1) 2026-07-28, A/B NULL — ⭐ **REBUILD DECIDED 2026-07-31, top of the list**; ✅ all 4 research gates ANSWERED 07-31; ⚖️ freeze LIFTED 08-31 (checklist 87); ⭐ DESIGN SPEC WRITTEN 2026-09-01 (`reports/DRONE_REBUILD_DESIGN_20260901.md` — V-a view tiers, tier table, `EF-074` guard, zero new persisted names) + build brief `prompts/DRONE_REBUILD_BUILD_high.md`; NOTHING BUILT — gated on a 1.1.0 re-base, then the owner's ratification and E-4/E-8 (spec §9); ⚖️ RETIRED 2026-09-17 (owner) — **PARKED, not dead**: module DELETED from the pack with its `items.lua` + `metadata.lua` entries (6 `Code/*.lua` files, 5 registered modules, the two sets agree). 1.1.0 deleted `DroneControl:CalcLapTime`, the instrument every A/B number here came from, so the desk cannot say whether the module still helps; the owner parked it rather than judge it. Its two allowlisted wrap sites left with the file (wrap allowlist 3 → 1). Code: `git show cc846e4:Code/Opt_DroneOverhaul.lua`. ⛛ Status word KEPT at `built` per the hotfix-2 precedent — retirement does not un-build what was built. ⛔ D06 had no probe of its own, but the TestKit's `OptionsMenuOptIn` WANT list still names `DroneOverhaul` — shared kit, kit-lane work under tools/TESTKIT.md"
updated: "2026-09-17"
copies: []
donor_seq: 104
donor_row: 69
...
- **The build brief is `docs/agent/prompts/DRONE_REBUILD_BUILD_high.md`** and it does not start until
...
the fix pack's `PLAYTEST_CHECKLIST.md` §1). **PT-52 Triggers A/B/B2 are FROZEN pending
...
## ⛔ 2026-09-17 — 1.1.0 REBUILT THE MACHINERY AROUND THIS MODULE: THE HOLE REMAINS, THREE ASSUMPTIONS MOVED, AND `DroneReport()` NOW THROWS
...
revalidation pass. No code changed; the drones are UNFROZEN (owner 2026-08-31) but nothing was
touched. Report: `docs/agent/reports/MODULE_REVALIDATION_1_1_0.md` §5. Owner ask:
`docs/PLAYTEST_CHECKLIST.md` **OI-05**.

**The hole this module was built for is still open.** `TaskRequestHub:FindTask` is byte-identical
(`1.1.0.403908/Src/Lua/_TaskRequest.lua:74-85`). The registration layer is unchanged: a building
still connects to EVERY covering hub via `FindDroneNodes` + `GetCommandCenter`
(`1.1.0.403908/Src/Lua/_TaskRequest.lua:286-317`; `FindDroneNodes` gained an optional `building`
argument defaulting to `self`, so `src:FindDroneNodes()` still works), and
`DroneHubExtenderBase:GetCommandCenter` is unchanged
(`1.1.0.403908/Src/Lua/Buildings/DroneHubExtender.lua:166-170`). **There is still no cross-hub
locality for WORK requests anywhere in 1.1.0.**

**The F86 tier-2 call position still holds.** `Drone:Idle`'s last two statements are still
`Sleep(2000)` then `self:CleanUnreachables()` (`1.1.0.403908/Src/Lua/Units/Drone.lua:653-712`),
every found-work branch still `SetCommand`s before reaching it, `Drone:CleanUnreachables` is still
declared (`:971`), and its other two call sites are still inside `Deliver` (`:1454`) and
`PickRechargeStation` (`:1491`) — so `self.command == "Idle"` still selects exactly the
fall-through.

**⛔ FOUR THINGS MOVED.**

1. **A SECOND `FindTask` PATH — the header's "ONLY caller" claim is now false.** The `FindTask` call
   moved into `Drone:TryTakeTask(command_center)`
   (`1.1.0.403908/Src/Lua/Units/Drone.lua:593-615`), which has TWO callers: `Idle` (`:707`) and the
   new `Drone:TryTakeTaskOnTheWay` (`:638-651`, called from `:730` and `:1756`) — a travelling drone
   taking a request that appears mid-trip, polled every `const.DroneTaskOnTheWayInterval` = 1000 ms
   (`1.1.0.403908/Src/Lua/_GameConst.lua:86-87`). `Code/Opt_DroneOverhaul.lua:23-25` states *"its
   ONLY caller is the drone auto-Idle path"*. The claim gate now also vetoes on-the-way polls. Not a
   correctness break — vetoes remain strike-bounded — but strikes can now burn at ~1/second per
   request, so `STRIKES_MAX = 4` may expire in seconds and the gate's effect may wash out.
2. **`GetIdleDronesCount` CHANGED MEANING.** It is now an alias for a new `GetFreeDronesCount`,
   which counts `Idle`, `WaitingCommand` **and any travelling drone eligible for an on-the-way
   task** (`1.1.0.403908/Src/Lua/Buildings/DroneControl.lua:992-1005`). This module uses it in
   opposite directions: `closest:GetIdleDronesCount() > 0` to veto
   (`Opt_DroneOverhaul.lua:189`) — now more eager; and `hub:GetIdleDronesCount() == 0` to detect a
   SATURATED hub worth moonlighting for (`:235`) — now much harder to satisfy, so **part 2 may
   effectively stop firing** on a busy colony.
3. **VANILLA ADDED TASK REASSIGNMENT — FOR HAULING ONLY.** `Drone:TryTaskSwap`
   ...
   closer to the other's target (`const.DroneTaskSwapDistancePct` / `MinGain`,
   `_GameConst.lua:89-92`). It is **PickUp-only** (`self.command ~= "PickUp"`, requires `s_request`)
   and **same-hub-only** (`other.command_center == command_center`). ⇒ It does NOT touch WORK
   requests and does NOT cross hubs, so part 1 is not overtaken. What it DOES land on is the half
   this module declared out of scope — *"PickUp/Deliver hauling … deliberately out of v1 scope"*.
   That is an input to the rebuild spec, not a verdict on the shipped module.
4. **⛔ THE TELEMETRY NOW THROWS.** `DroneControl:CalcLapTime`, `const.DroneLoadLowThreshold` and
   `const.DroneLoadMediumThreshold` **do not exist anywhere in the 1.1.0 tree** (they were at
   `1.0.7.396349/Src/Lua/Buildings/DroneControl.lua:955, 998, 1000`). 1.1.0 measures hub load as a
   sampled idle percentage over a window (`const.DroneLoadSampleInterval`, `DroneLoadWindow`,
   `DroneLoadHighPct`, `DroneLoadMediumPct`, `1.1.0.403908/Src/Lua/_GameConst.lua:94-98`).
   `SMROptInPack.DroneReport()` calls `hub:CalcLapTime()` unconditionally
   (`Code/Opt_DroneOverhaul.lua:293`) and is registered **whether or not the module is enabled**, so
   invoking it on 1.1.0 raises `attempt to call a nil value (method 'CalcLapTime')`. Console-invoked
   only, so it cannot fire by itself — but it is this module's ONLY instrument.

**Two smaller drifts.** `Drone:Idle` now gates on `command_center:CanCommandDrones()`
(`1.1.0.403908/Src/Lua/Buildings/DroneHub.lua:150-153` — a hub without power still commands drones)
where this module still tests `.working` in three places, so the module is now narrower than
vanilla's own notion of a working hub. And `const.DroneRestrictRadius` gained an underground sibling
`const.DroneRestrictRadiusUnderground = CommandCenterMaxRadius * 3 * GridSpacing`
(`_GameConst.lua:73`) selected by a new `GetDroneRestrictRadius(map)`
(`1.1.0.403908/Src/Lua/Units/Drone.lua:232-237`); this module hardcodes the surface constant in both
reach tests, so on an underground map it under-states drone reach by a third.

**⚠️ THE REBUILD SPEC WAS WRITTEN AGAINST 1.0.7.**
`reports/DRONE_REBUILD_DESIGN_20260901.md` and its build brief
`prompts/DRONE_REBUILD_BUILD_high.md` predate 1.1.0 by a week, and the experiments they gate on
(E-4/E-8, spec §9 ask 4) were specified against the machinery above. Re-read both against the
1.1.0 tree before the spec is put to the owner for ratification.

**Control — what would falsify this.** With the module ON on 1.1.0 and telemetry repaired: park idle
drones beside a building a distant hub also covers, break it, and see which fleet claims the repair —
if the near fleet already wins with the module OFF, vanilla acquired locality somewhere this read
missed. Then read `vetoed` vs `veto_expired`: if `veto_expired` dominates, the on-the-way polls are
burning the strike budget as predicted. Then read `moonlighted`: 0 across a session with a visibly
saturated hub confirms the `GetIdleDronesCount` redefinition disabled part 2. None of this has been run.

**What this does NOT claim.** No claim that the module is harmful on 1.1.0 — it is not shown to be.
No claim about the A/B numbers (the 88 %-of-repair-time figure, the hub-load bands): the instrument
that produced them no longer exists, so they are 1.0.7 measurements and nothing more. No claim about
`Fix_ExtenderFlapChurn`, which lives in the fix pack.

---

## ⚖️ 2026-09-17 — RETIRED by owner ruling: **PARKED**, and the module file is DELETED

**The ruling.** The owner ruled the `OI-05` ask (`docs/PLAYTEST_CHECKLIST.md`, closed by that ruling):
**park `Opt_DroneOverhaul`** rather than judge it. PARKED is not DEAD. Nothing here says the module
stopped helping — it says the desk cannot tell, because 1.1.0 removed the instrument that produced
every number this entry cites.

**Why it could not be judged from the desk.** The working is the section immediately above and §5
of `../reports/MODULE_REVALIDATION_1_1_0.md`. In short, against
`C:\Dev\SMR-SrcArchive\1.1.0.403908\Src`:

- ⛔ `DroneControl:CalcLapTime` and both `const.DroneLoad*Threshold` values **no longer exist** —
  the A/B instrument is gone, so the 1.0.7 measurements cannot be reproduced or compared.
- `TaskRequestHub:FindTask` gained a **second caller polled about every 1000 ms**
  (`Drone:TryTakeTaskOnTheWay`), so the module's `STRIKES_MAX = 4` budget can burn in seconds.
- `GetIdleDronesCount` became an alias for `GetFreeDronesCount` and now counts travelling drones,
  so the saturated-hub test that triggers moonlighting may never be satisfied again.
- Vanilla added **same-hub PickUp task swapping** (`Drone:TryTaskSwap`) — it does not overtake this
  module, but it lands on ground the module declared out of scope.

**The hole is still there.** `TaskRequestHub:FindTask` is byte-identical and there is still no
cross-hub locality for WORK requests anywhere in 1.1.0. That is why this is a park and not a death.

**What left the tree.** `Code/Opt_DroneOverhaul.lua` deleted; its `ModItemCode` and
...
modules**, the two sets agree (`python tools/doccheck.py --emit-counts`; never hand-typed). The
module's two allowlisted wrap sites (`Drone.CleanUnreachables`, `TaskRequestHub.FindTask`) left with
the file, so the wrap allowlist is **3 → 1**, and `SMROptInPack.DroneReport()` went with it, which
disposes of the `CalcLapTime` throw described above.

**Where the code is.** ⛔ **Git is the record.** Restore sha **`cc846e4`** — the last commit in which
this module still shipped: `git show cc846e4:Code/Opt_DroneOverhaul.lua`. A convenience copy sits at
`C:\Dev\SMR-OptInPack-archive\PARKED-Opt_DroneOverhaul.lua` — deliberately OUTSIDE the mod root, so
the Mod Editor cannot sweep it into an upload pack and the junctioned game folder cannot see it,
untracked, with restore steps in its `README.md`. It is a copy, not the record.

**What reviving it would take.** Not an un-delete. The rebuild spec
(`../reports/DRONE_REBUILD_DESIGN_20260901.md`) and its build brief
(`../prompts/DRONE_REBUILD_BUILD_high.md`) were written against 1.0.7 machinery and have to be re-based
on 1.1.0 first, and a replacement for `CalcLapTime` has to exist before any claim about the module's
value can be measured at all. ⚠️ Only after that re-base do the rebuild's owner asks go on
`docs/PLAYTEST_CHECKLIST.md`, re-specified for 1.1.0: they are the spec's §9 asks 1–5 — ratify the
mechanism and tier table (answering the bands report's §4.3 shortlist), food-service default
priority 3, repair moonlighting, the E-4/E-8 matcher experiments (bands report §5) with tiers on
rockets and rovers or hubs only (bands report §4.2), and the disclaimer's player-facing wording
(spec §7). That caveat was raised with OI-05 and survives its closure.

**Persisted names.** This module wrote none: its caches were module locals with weak keys
(`../FIX_POLICY.md` §3). Its Mod-Options toggle key `DroneOverhaul` is row 8 of the persisted-name
inventory and **stays there** — those bytes are contract and history. A stale key left behind in
`AccountStorage.ModOptions` is inert; see §10 of `../reports/MODULE_REVALIDATION_1_1_0.md`.

**Status word.** KEPT at `built`, per the hotfix-2 precedent the fix pack set on 2026-09-12
(its `F43`/`F118`). Retirement does not un-build what was built, and nothing above was measured.

**TestKit — orphaned, FLAGGED, not edited.** D06 never had a probe of its own (`READINESS_REVIEW_0831`
§5), but `OptionsMenuOptIn`'s WANT list still names `DroneOverhaul`
(`C:\Dev\SMR-BugFixPack-TestKit\Code\60_Probes_Opt.lua:892-901`). The kit is **SHARED** with the
fix pack; kit work follows `tools/TESTKIT.md` and `tools/SMRTK.md` (the fix pack's permissions) and
is never committed from a pack lane, so removing this orphaned probe is kit-lane work, not an
owner decision.
