...
`local/retired-modules/PARKED-Opt_DroneOverhaul.lua` — git-ignored (`metadata.lua`'s `ignore_files`
keeps `local/` out of every upload pack; doccheck's PACK IGNORE PARITY gate holds it),
