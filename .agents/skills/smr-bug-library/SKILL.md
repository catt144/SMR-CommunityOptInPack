...
`docs/agent/bugs/` holds this mod's **module records**, including retired modules and the
...
1. **`docs/agent/bugs/INDEX.md`** — small. One row usually answers "is this
   known, and what is its status". Read the row, not the entry.
2. **The entry's own section, by heading.** Never read a whole entry for a narrow question.
3. `docs/agent/facts/INDEX.md` is large — **grep it, never read it whole**. To ask whether a
...
`row_status:` is a frozen copy of the pre-split index row. doccheck
